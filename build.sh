...
# [삭제] 환경 변수 설정 (cron으로 실행할 경우에만 필요)
...
if [ $? -eq 0 ]; then
    ...
    # [삭제] WAR 파일 위치 (내부에서는 eCams가 수행)
    WAR_FILE=$(find build/libs -name "*-plain.war" | head -n 1)
    if [ -z "$WAR_FILE" ]; then
        record_time "WAR 파일을 찾을 수 없습니다. 빌드가 성공했는지 확인하세요."
        exit 1
    fi

    # [삭제] Tomcat 웹 애플리케이션 디렉토리로 WAR 파일 복사 (내부에서는 eCams가 수행)
    TOMCAT_WEBAPPS_DIR="/opt/tomcat/webapps"
    record_time "Tomcat 웹 애플리케이션 디렉토리로 WAR 파일을 복사하는 중..."
    cp "$WAR_FILE" "$TOMCAT_WEBAPPS_DIR/epams.war"

    # [삭제] Tomcat 서비스 재시작 (내부에서는 eCams가 수행)
