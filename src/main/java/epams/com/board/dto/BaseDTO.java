...
public class BaseDTO {
    ...
    public BaseDTO() {
        ...
        this.guidPrgSno  = 0;
        this.lstChgUsid = new String(new char[14]).replace('\0', '0');
        this.updatedTime = LocalDateTime.now();  // 현재 시간으로 초기화
        this.createdTime = LocalDateTime.now();  // 현재 시간으로 초기화
    }

    // 모든 필드를 매개변수로 하는 생성자
    public BaseDTO(String deleteYn, String guid, int guidPrgSno, String lstChgUsid, LocalDateTime updatedTime, LocalDateTime createdTime) {
